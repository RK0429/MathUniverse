...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | **Zero-friction capture** — commit Lean files anywhere in the repo; saving them in VS Code auto-triggers `lake exe lean_stmt_export` (Lean) & `cargo run -p mdx-gen` (Rust) | VS Code extension publishes file-save events and can run arbitrary tasks – Lean/Rust-specific tasks can be configured. ([GitHub](https://github.com/leanprover/vscode-lean4?utm_source=chatgpt.com))                                                                                                                                                                                                                                                                                                                                                               |
| 2   | **Automatic article generation** with Lean (JSON export) + Rust (Tree-sitter parsing & MDX templating)                                                                      | Lean metaprogramming (`lake exe lean_stmt_export`) exports `stmt_deps.json` (containing declaration `id`, `kind`, and `deps`). A Rust binary (`mdx-gen`) parses this JSON, uses Tree-sitter to find `example` blocks in `.lean` files, and renders final MDX pages using Tera/Handlebars templates. This replaces `doc-gen4`. ([Metaprogramming in Lean 4](https://leanprover-community.github.io/lean4-metaprogramming-book/main/02_overview.html?utm_source=chatgpt.com), [tree-sitter-lean](https://github.com/Julian/tree-sitter-lean?utm_source=chatgpt.com)) |
...
flowchart TD
  ...
  subgraph Build
    LeanFile -- Lean Metaprogramming via 'lake exe lean_stmt_export' --> JsonOutput(stmt_deps.json)
...
jobs:
  build:
    ...
    steps:
      ...
      - run: lake exe lean_stmt_export # Lean metaprogramming: exports stmt_deps.json (id, kind, deps)
      - run: cargo run -p mdx-gen     # Rust: processes JSON & .lean files to generate .mdx
...
The flow fails fast on Lean errors; the cache prevents a full mathlib recompile on every run. The Rust step (`mdx-gen`) should be built or available. ([Metaprogramming in Lean 4](https://leanprover-community.github.io/lean4-metaprogramming-book/main/02_overview.html?utm_source=chatgpt.com), [Tera](https://keats.github.io/tera/docs/?utm_source=chatgpt.com), [Handlebars-Rust](https://github.com/sunng87/handlebars-rust?utm_source=chatgpt.com))
...
1. **Install Lean 4 & set up Lean metaprogramming export** (`lake exe lean_stmt_export` to produce `stmt_deps.json` detailing `id`, `kind`, and `deps` for each declaration). ([Lean Metaprogramming Book](https://leanprover-community.github.io/lean4-metaprogramming-book/main/02_overview.html?utm_source=chatgpt.com))
...
3. **Develop Rust crate (`mdx-gen`)** for JSON parsing (`serde`), Lean code parsing (`tree-sitter-lean`), and MDX templating (`Tera` or `Handlebars`). ([tree-sitter-lean](https://github.com/Julian/tree-sitter-lean?utm_source=chatgpt.com), [Tera](https://keats.github.io/tera/docs/?utm_source=chatgpt.com))
