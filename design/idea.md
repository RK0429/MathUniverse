...
| 2   | **Automatic article generation** with Lean (JSON export) + Rust (Tree-sitter parsing & MDX templating)                                                              | Lean metaprogramming (`lake exe genDecls`) exports `declarations.json`. A Rust binary (`mdx-gen`) parses this JSON, uses Tree-sitter to find `example` blocks in `.lean` files, and renders final MDX pages using Tera/Handlebars templates. This replaces `doc-gen4`. ([Metaprogramming in Lean 4](https://leanprover-community.github.io/lean4-metaprogramming-book/main/02_overview.html?utm_source=chatgpt.com), [tree-sitter-lean](https://github.com/Julian/tree-sitter-lean?utm_source=chatgpt.com)) |
...
jobs:
  build:
    ...
    steps:
      ...
      - run: lake exe genDecls         # Lean metaprogramming: exports declarations.json
...
1. **Install Lean 4 & set up Lean metaprogramming export** (`lake exe genDecls` to produce `declarations.json`). ([Lean Metaprogramming Book](https://leanprover-community.github.io/lean4-metaprogramming-book/main/02_overview.html?utm_source=chatgpt.com))
